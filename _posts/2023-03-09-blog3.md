...
式中，$$r’$$表示现在到达$$P$$的信号，在从电荷出发时与$$P$$点的距离。这个公式的推导还是要等后面进入场的学习的时候进一步思考，现在我们只是单纯分析这一公式所导出的结果。显然在上式的诸多项中，第一项与第二项都是随着距离的平方衰减的项，而第三项随距离的一次方衰减。<ins>当我们离得足够远时</ins>，第三项是唯一重要的项：
...
于是，我们发现，板的效果实际上式相对于场乘上了一个相位因子。但我们更希望写成附加的形式来独立地分析玻璃引入的场，所以，**当$$\bigtriangleup z$$非常小的情况**，我们把附加的相位因子改写成如下形式：
