...
而量子振子的能量应该为：$$\left \langle E \right \rangle=\frac{E_总}{N_总}=\frac{N_0\hbar \omega (0+p+2p^2+3p^3+...)}{N_0(1+p+2p^2+3p^3+...)}=\frac{\hbar \omega}{e^{\hbar \omega/kT}-1}$$,其中$$p$$是每上升一个能级该能级粒子概率的变化系数：$$p=e^{-\hbar \omega/kT}$$。
